/* The driver is modified for easy testing of different setups */
...
#define dev_vdbg(dev, fmt, arg...)      \
        if (debug > 1)                  \
			dev_info(dev, fmt, ##arg)

static unsigned int debug = 0;
module_param(debug, uint, 0);
MODULE_PARM_DESC(debug, "0-2");
...
static struct ads7846_platform_data pdata_ads7846_fbtft = { 0, };

static struct spi_board_info spi_ads7846_fbtft = {
	.modalias = DRVNAME,
	.platform_data = &pdata_ads7846_fbtft,
};
...
static int __init ads7846_init(void)
{
	...
	struct ads7846_platform_data *pdata = &pdata_ads7846_fbtft;

	if (debug)
		pr_info("\n\n"DRVNAME": %s()\n", __func__);

	/* set SPI values */
	spi_ads7846_fbtft.max_speed_hz = speed;
	spi_ads7846_fbtft.bus_num = busnum;
	spi_ads7846_fbtft.chip_select = cs;
	spi_ads7846_fbtft.mode = mode;
	irq = irq ? : (GPIO_IRQ_START + gpio_pendown);
	spi_ads7846_fbtft.irq = irq;
	...
	master = spi_busnum_to_master(spi_ads7846_fbtft.bus_num);
	if (!master) {
		pr_err(DRVNAME":  spi_busnum_to_master(%d) returned NULL\n", spi_ads7846_fbtft.bus_num);
		return -EINVAL;
	}

	spidevices_delete(master, spi_ads7846_fbtft.chip_select);      /* make sure it's available */

	ads7846_spi_device = spi_new_device(master, &spi_ads7846_fbtft);
